...
async function loadImage(filepath: string) {
	let data, info;
	try {
		({ data, info } = await sharp(filepath)
			.ensureAlpha()
			.raw()
			.toBuffer({ resolveWithObject: true }));
	} catch {
		printError(`Could not load image ${filepath}`);
		process.exit(1);
	}
...
cv.onRuntimeInitialized = async () => {
	...
	if (!fs.existsSync(img1Path)) {
		printError(`Could not find image ${img1Path}`);
		process.exit(1);
	}
	if (!fs.existsSync(img2Path)) {
		printError(`Could not find image ${img2Path}`);
		process.exit(1);
	}
	...
	if (numToBlend == 2) {
		...
		if (!fs.existsSync(img3Path)) {
			printError(`Could not find image ${img3Path}`);
			process.exit(1);
		}
		if (!fs.existsSync(img4Path)) {
			printError(`Could not find image ${img4Path}`);
			process.exit(1);
		}
